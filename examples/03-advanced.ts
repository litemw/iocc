...
const IPlugin = defIntf<Plugin>('Plugin').multi;
const IApp = defIntf<App>('App');

const AuthPlugin = defComp('authPlugin').as(IPlugin)(() => {
  return {
    name: 'auth',
    run: () => 'auth ready',
  };
...
// Another component can contribute to the same interface.
const MetricsPlugin = defComp('metricsPlugin').as(IPlugin)(() => {
  return {
    name: 'metrics',
    run: () => 'metrics ready',
  };
...
const AuditPlugin = defComp('auditPlugin')(() => {
  return {
    name: 'audit',
    run: () => 'audit ready',
  };
...
const AppComponent = defComp('app').provide(ILogger.optional, IPlugin).as(IApp)(
  (logger, plugins) => {
    return {
      boot() {
